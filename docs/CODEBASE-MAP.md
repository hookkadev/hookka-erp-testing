...
| `src/pages/dashboards/SitiOpsView.tsx` — draft "Operations (Siti)" tab: overdue-by-department, due-within-3-days, Daily Production Output chart, Plan vs Actual table, Production Cost chart (material+labor, stacked, by completion date), Material Shortage table | ↳ same feed, `production` (`overdueByDept`/`dueSoon3Days`/`dailyOutput`/`planVsActual`/`productionCost`) + `inventory.materialShortage` | `production_orders` / `fg_batches` / `raw_materials` | none yet |
