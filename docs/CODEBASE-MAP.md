...
> **Last verified: 2026-09-24 on branch `feat/pi-line-discount-main`** — the Procurement gotcha "PI per-line discount" only (new, DEV-14), and the `checkPoRemaining` anchor.
>
...
- **PI per-line discount (DEV-14, 2026-09-24)**: `purchase_invoice_items.discount_sen` (self-applied in `ensurePiMigrations`). `line_total_sen` is stored NET of it via `discountedLineSen` (`src/lib/unit-price.ts`, clamped to [0, gross]) — the API, create page, detail page and scan modal all call it, so every `line_total_sen` reader (GL legs, costing, 3-way match, AP) sees the discounted amount unchanged. Distinct from the `DISCOUNT` line TYPE, which is a separate line and (pre-existing) ADDS to the total. Scan: `sanitizeSupplierDoc` adds the discount back when it backs a unit price out of a net amount. Tests: `tests/pi-line-discount.test.mjs`.
