...
| `src/pages/dashboards/SitiOpsView.tsx` — draft "Operations (Siti)" tab: overdue-by-department, due-within-3-days, Daily Production Output chart, Plan vs Actual table, Material Shortage table, Production Cost stub (not wired) | ↳ same feed, `production` (`overdueByDept`/`dueSoon3Days`/`dailyOutput`/`planVsActual`) + `inventory.materialShortage` | `production_orders` / `raw_materials` | none yet |
