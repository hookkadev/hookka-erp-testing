...
>
> **Last verified: 2026-09-23 on branch `ci/github-deployments-status`** — the Service & Repair `service-cases/index.tsx` row only: line count re-measured (`wc -l`, 1672; was 1522) and the Source-column behaviour noted.
...
| Frontend page | API route | Primary tables | Tests |
|---|---|---|---|
| `src/pages/service-cases/index.tsx` — Service Cases list; Source column shows `EXTERNAL (<externalRef>)` for external cases (1672). Last verified 2026-09-23 | `src/api/routes/service-cases.ts` — service_cases CRUD + status + photos + stock top-ups (959) | `service_cases` / `service_orders` / `service_order_lines` / `service_order_returns` | `tests/case-pipeline.test.mjs` |
