...
- **Efficiency % is DAY-GATED against clocked production hours (2026-09-22, BUG-2026-09-22-004).** A completed card's minutes count for a worker ONLY on a `completedDate` where that worker also has a production-dept `working_hour_entries` row — in `/api/job-cards/summary` (EXISTS gate on each pic branch; `working_hour_entries` now in its snapshot `sourceTables`) and in `department-performance.ts` (per-worker JC + ADD_PROD credit skipped on unclocked days; `day.productionMinutes` DERIVED from the gated per-worker cells). This closes the numerator/denominator span mismatch that let Efficiency % read >100% (the 428% case) when Working Hours weren't fully entered; a fully-entered period is a no-op. Non-prod hours are excluded on both sides. Prod-impact UNMEASURED at merge — `scripts/check-efficiency-day-gate.mjs` prints OLD vs NEW per worker for whoever holds `HOOKKA_PROD_DB_URL`.
