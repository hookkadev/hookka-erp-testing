...
|---|---|---|---|
| `src/pages/sales/index.tsx` — SO list (2181), dual-mode SO vs service-order | `src/api/routes/sales-orders.ts` — 5,865 lines (re-measured 2026-09-23; create POST now WARNS, not 409s, on a repeated customer PO/SO ref — BUG-2026-09-25-195) (+ `sales-orders/_helpers.ts`, 1,462); SO CRUD + status cascades + snapshot | `sales_orders` / `sales_order_items` / `so_status_changes` | `tests/sofa-combo.test.mjs` |
...
| `src/pages/production/sequence-unlocks.tsx` — weekly sequence-unlock report (who / which step / real skip vs recording gap; PRD T-013 R12, "Unlock report" button on the production header) | `GET /api/production-orders/sequence-unlocks?days=7` (`src/api/routes/production-orders.ts:444`) + `src/api/routes/sheets-sync.ts` — the Sheets webhook now gated too | `scan_override_audit` (+ `reason_code` / `department_code` / `blocked_by` / `actor_kind`, self-applied) | `tests/sequence-lock-side-doors.test.mjs` · `tests/sequence-unlock-reasons.test.mjs` · `tests/ordered-batch.test.mjs` |
...
- `dueFrom` / `dueTo` do **NOT** filter `targetEndDate`, whatever the comment above them at `src/api/routes/production-orders.ts:851` says (it still reads "Server-side targetEndDate window"; the vars are at `:809`). Since 2026-06-10 the OVERVIEW branch (no `dept`) windows the PO's SO `hookkaExpectedDD` (`ddExpr` in `_helpers.ts`) and the DEPT branch windows that dept's `job_cards.dueDate`. Undated rows pass on both sides deliberately. Also note there is no far-future tail to trim on this dataset — every active PO's expected DD sits inside ~7 months, so a `dueTo` horizon removes ~0.2 % of rows and is not a perf lever (measured 2026-08-13).
...
- **`src/api/lib/sequence-lock.ts` — one answer to "may this job card move yet?" (2026-09-07).** Pure: no DB, no department table. `sequenceBlockers(card, allCards)` returns the cards that must finish first, read off the BOM's own shape — a card waits for every earlier-`sequence` card in its own `(wipKey, branchKey)`, and a card whose `branchKey` is EMPTY (the BOM's mark for a step belonging to the whole product) also waits for the LAST card of every other branch in that `wipKey`. Change a BOM and this follows; there is nothing to maintain. It landed on main AHEAD of the gate that refuses with it (#431), because `scripts/backfill-skipped-completions.mjs` has to close exactly the cases the gate rejects. **The gate is wired since 2026-09-17 (PRD T-013, BUG-2026-09-25-195):** ONE function, `gateJobCardSequence` (`src/api/routes/production-orders/_helpers.ts:6100`), called from `applyPoUpdate` (`src/api/routes/production-orders/_helpers.ts:4379`), from the sticker scan and both fan-out scans via `gateFanOutSequence` (`src/api/routes/production-orders.ts:2629`), and from the Google Sheets webhook (`src/api/routes/sheets-sync.ts`); it refuses with `409 UPSTREAM_INCOMPLETE`, decides `canSelfUnlock` in `canSequenceUnlock`, validates the reason (`src/api/lib/sequence-unlock-reasons.ts`) and records the real actor. The nine bulk status writers in `import-completion/*` are `requireAdmin`. `tests/sequence-lock-side-doors.test.mjs` enumerates every `job_cards` status writer in `src/api`. See the row in the big-file index below and `docs/modules/production.md` flow 7. Never read `PRODUCTION_ORDER_BY_WIP_TYPE` from here (build-time ordering) and never `job_cards.prerequisiteMet` (stamped once at build, never rolled forward — measured 2026-09-06: at least 2,611 of 4,967 cards on active orders read it wrongly). Tests: `tests/sequence-lock.test.mjs` (the interim `sequence-rule-unit.test.mjs` was folded into it once the gate landed).
...
|---|---|---|---|---|
| `src/api/lib/sequence-lock.ts` | 170 | The ONE answer to "may this job card move yet?" — a job card waits for every earlier-`sequence` card in its own `(wipKey, branchKey)`, and a card whose `branchKey` is EMPTY (the BOM's marker for a step belonging to the whole product, set by `bom-wip-breakdown.ts` when it walks the tree) additionally waits for the LAST card of every other branch. **Pure — it reads job cards and nothing else.** No table, no constant: the sequence and branch were stamped from the BOM at build time, so changing a BOM changes the lock for the next cards built. It must never import `DEPT_ORDER` / `PRODUCTION_ORDER_BY_WIP_TYPE` (a test forbids it) — that flat order lists WOOD_CUT after FAB_SEW and would block wood cutting on fabric sewing. It must also never read `job_cards.prerequisiteMet`, which is stamped once at creation and never rolled forward (measured 2026-09-06: 4,680 of 4,967 active cards read 0, at least 2,611 falsely). Callers, all through the ONE gate `gateJobCardSequence` (`production-orders/_helpers.ts:6100`): `applyPoUpdate` (covers the grid AND bulk-patch, which loops back into it), `POST /:id/scan-complete`, both fan-out scans via `gateFanOutSequence`, and the Google Sheets webhook. Refusal is `409 { code: "UPSTREAM_INCOMPLETE", blockedBy, blockedCards, canSelfUnlock }`; `canSelfUnlock` is `canSequenceUnlock`'s decision, never the client's. | 0 (pure lib) | `tests/sequence-lock.test.mjs`, `tests/sequence-lock-side-doors.test.mjs` |
| `src/api/lib/sequence-unlock-reasons.ts` | 81 | The reason vocabulary for releasing the sequence lock — `UNLOCK_REASON_OPTIONS` (code + label), `validateUnlockReason` (required, 3–300 chars, bare "Other" refused), `classifyUnlockReason`, `isRecordingGap`. Shared by the server gate and both pickers (desktop dialog, worker phone) so the weekly report groups one list. Client re-export: `src/lib/sequence-unlock.ts`. | 0 (pure lib) | `tests/sequence-unlock-reasons.test.mjs` |
| `src/api/lib/ordered-batch.ts` | 40 | `runGroupedInOrder(items, keyOf, runOne)` — sequential within a key, keys concurrent. bulk-patch uses it keyed by production order so "complete the earlier step too" finishes the upstream card before starting the blocked one (the old `Promise.all` kept list order and lost execution order — PRD T-013 R6). | 0 (pure lib) | `tests/ordered-batch.test.mjs` (observes execution order) |
