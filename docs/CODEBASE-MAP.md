...
| `src/pages/dashboards/EmployeesView.tsx` — headcount + "Overall Efficiency" (total production minutes ÷ total working minutes via `overallEfficiencyPct` in `dashboard-shared-lib.ts`, a weighted total, NOT an average of people and NOT `attendance_records.efficiency_pct`; see note below) KPIs, working-vs-production-hours chart (scoped to the global period picker), worker table. Denied (no `workers:read`) renders via `availability.employee` same as a query failure | ↳ same feed, `employee` / `availability.employee` slice only | `workers` / `working_hour_entries` / `job_cards` | none yet |
| `src/pages/dashboards/dashboard-shared.tsx` — `PeriodPicker` (Day/Month/YTD toggle + arrows + its trigger popover: `CalendarGrid` for Day, `MonthGrid` for Month, none for YTD), `Kpi` (no icon since 2026-09-25: label, value, then sub; a `+x% / -x%` sub renders as a green/red pill, any other sub as plain wrapping text), `LiveBadge`, `MissingNote`, `SnapshotNote` | | `attendance_records` / `working_hour_entries` / `workers` | |
| `src/pages/dashboards/dashboard-shared-lib.ts` — chart theme tokens + the `Period` model (`inPeriod` / `previousPeriod` / `periodLabel` - `periodLabel` takes an optional `today` to sharpen the YTD label / `stepDay` / `yearsWithData`) + `overallEfficiencyPct` (Employees + Operations efficiency, test `tests/dashboard-period.test.mjs`) + `dayLabel` / `dayList` and `warnDays` (the dates behind a time-audit warning, used by `EmployeesInsights.tsx` and `/m` `PeopleTab.tsx`) + the truncating widget formatters (`truncDp` / `fmtDec2` / `fmtPct2` / `fmtRM2` / `fmtHM`) and `widgetPeriod` / `widgetPeriodLabel`. Split from the `.tsx` so Fast Refresh keeps working | | | |
