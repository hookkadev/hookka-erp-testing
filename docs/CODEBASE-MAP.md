...
- **`/production` at ≤1280px is the `/m` card list (2026-09-22):** `src/pages/production/overview.tsx` checks `useMediaQuery("(max-width: 1280px)")` and mounts `ProductionScreen` from `src/pages/m/screens/ProductionScreen.tsx` (lazy) instead of the matrix. The Overview matrix is a fixed-px grid with a ~1,980px minimum and does NOT reflow — it is desktop-only by design. There is ONE small-screen layout (the /m screen); don't add a responsive mode inside `index.tsx`. Card tap → `/m/production/:id` (desktop has no PO-detail route). Dept pages (`dept.tsx`) are unaffected.
