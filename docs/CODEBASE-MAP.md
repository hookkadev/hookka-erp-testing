...
| `src/pages/dashboards/dashboard-shared-lib.ts` — chart theme tokens + the `Period` model (`inPeriod` / `previousPeriod` / `periodLabel` - `periodLabel` takes an optional `today` to sharpen the YTD label / `stepDay` / `yearsWithData`) + `dayLabel` / `dayList` and `warnDays` (the dates behind a time-audit warning, used by `EmployeesInsights.tsx` and `/m` `PeopleTab.tsx`) + the truncating widget formatters (`truncDp` / `fmtDec2` / `fmtPct2` / `fmtRM2` / `fmtHM`) and `widgetPeriod` / `widgetPeriodLabel`. Split from the `.tsx` so Fast Refresh keeps working | | | |
