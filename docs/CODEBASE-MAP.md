...
> (1197 → 1069 lines, after its dead Transfer-to-DO dialog was deleted). Second pass the same
> day: `node scripts/check-codebase-map.mjs` FAILED on the Convert-chain entry — the
> `checkPoRemaining` line ref had drifted 21 lines, and the claim beside it had gone stale with it
> (three callers → four, "aggregated per material code" → per bucket, no mention of the R5 DB
> backstop). All corrected; the checker is now OK. Added `src/pages/m/screens/DocumentDetailScreen.tsx`
> (update-on-touch — it gained the mobile GRN idempotency key), so the advisory coverage gap is
> 12 modules, all pre-existing on `main`. Rest of the map not re-verified in this pass.
>
...
| ↓ `src/pages/m/screens/DocumentDetailScreen.tsx` — the generic `/m` document detail screen (1904): record view, edit sheet, POD, and the **Goods Receipt** sheet (`GrnReceiveSheet`, `:1164`), which posts through the SAME `POST /api/grn` as the desktop receive form and sends an `Idempotency-Key` from `grnIdem` (`:137`) so a retry on a flaky phone connection cannot post the goods twice | `src/api/routes/grn.ts` — `POST /` (wrapped in `withIdempotency`, T-006 R10; born-POSTED receipts write header + stock + PO counter in ONE batch, R3) | `grns` / `grn_items` / `purchase_order_items` | `tests/t006-r10-client-key.test.mjs` |
