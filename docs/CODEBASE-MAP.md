...
| `src/pages/dashboards/dashboard-prototype.tsx` — page shell: tab strip + the GLOBAL period control (Monthly / YTD / datepicker) that every dated panel reads. Ships **three** tabs (All Overview, Sales Orders, Employees — `EmployeesView.tsx` added 2026-09-17, ported from the stale `laphii/feature/dashboard` copy onto this branch's current `employee`/`availability.employee` payload shape); the other four (Delivery, Inventory, Purchase, Production) are built on `laphii/feature/dashboard` | `src/api/routes/dashboard-prototype.ts` — single `GET /` (1715), mounted at `/api/dashboard/prototype` (`src/api/worker.ts:1307`), auth-gated, 60s KV-cached (2026-09-17, same `cached()` helper as dashboard-overview.ts — the route was ~15 sequential queries with no caching, ~1.5-3s cold). The per-request permission redaction (delivery/purchase/inventory/employee/production sections dropped per caller) runs OUTSIDE the cache, on every request, against the cached raw payload — never cache the redacted response itself. Returns all six sections in one payload | `sales_orders` / `sales_order_items` | none yet — see gap below |
