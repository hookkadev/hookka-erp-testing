...
|---|---|---|---|
| `src/pages/sales/index.tsx` — SO list (2181), dual-mode SO vs service-order | `src/api/routes/sales-orders.ts` — 5,865 lines (re-measured 2026-09-23; create POST now WARNS, not 409s, on a repeated customer PO/SO ref — BUG-2026-09-24-192) (+ `sales-orders/_helpers.ts`, 1,462); SO CRUD + status cascades + snapshot | `sales_orders` / `sales_order_items` / `so_status_changes` | `tests/sofa-combo.test.mjs` |
...
"Fulfillment pipeline"); Operations > Overview = Plant Load + Department Backlog (after
Overdue-by-dept), > Output = Completed, > Materials = Purchasing + Fabric Usage; Finance >
...
| `src/pages/dashboards/SalesOrdersView.tsx` (1279) — KPIs w/ deltas, revenue trend (bars = revenue, line = orders, click-to-drill), attribution by customer, 3-month rolling forecast, `/dashboard` widgets (Order Pipeline / Revenue trend / Sales by Customer / Top Sellers, from `DashboardWidgets.tsx`), state/category donut, Top SKUs | ↳ same feed, `sales` slice only | `raw_materials` / `rm_batches` / `purchase_orders` | |
| `src/pages/dashboards/OperationsView.tsx` — Operations tab (sub-tabs Overview / Output / Plan vs Actual / Revenue & Cost / Materials; Plan vs Actual and production revenue are mounted from `ProductionDailyPanels.tsx`, which also exports `DeptEfficiencyCard` for Employees > Efficiency), Overview redesigned 2026-09-18 for density: a 7-card KPI strip (Overdue / Due-3-Days / Plan-vs-Actual stage-completion % with mini progress bar / Material Shortage / Production Cost / Attendance / Efficiency with 7-day sparkline), a 2-col chart split (overdue-by-dept horizontal bar / Daily Production Output with a dashed 30-day-average reference line — NOT an owner-set target, none exists in the data), a full-width filterable (department pills + search) due-within-3-days worklist with color-coded urgency pills, then supporting detail tables (on-time completion, cost trend, shortage). `/dashboard` widgets (2026-09-23, `DashboardWidgets.tsx`): Overview = Plant Load + Department Backlog, Output = Completed, Materials = Purchasing + Fabric Usage | ↳ same feed, `production` (`orders` stagesDone/stagesTotal, `overdueByDept`/`dueSoon3Days`/`dailyOutput`/`planVsActual`/`productionCost`) + `inventory.materialShortage` + `employee` (attendance/performance, redaction-gated) | `production_orders` / `fg_batches` / `raw_materials` / `attendance_records` / `working_hour_entries` | none yet |
| `src/pages/dashboards/EmployeesView.tsx` — headcount + efficiency (Prod ÷ Working, NOT `attendance_records.efficiency_pct` — see note below) KPIs, working-vs-production-hours chart (scoped to the global period picker), worker table. Denied (no `workers:read`) renders via `availability.employee` same as a query failure | ↳ same feed, `employee` / `availability.employee` slice only | `workers` / `working_hour_entries` / `job_cards` | none yet |
| `src/pages/dashboards/dashboard-shared.tsx` — `PeriodPicker` (Day/Month/YTD toggle + arrows + its trigger popover: `CalendarGrid` for Day, `MonthGrid` for Month, none for YTD), `Kpi`, `LiveBadge`, `MissingNote`, `SnapshotNote` | | `attendance_records` / `working_hour_entries` / `workers` | |
| `src/pages/dashboards/dashboard-shared-lib.ts` — chart theme tokens + the `Period` model (`inPeriod` / `previousPeriod` / `periodLabel` - `periodLabel` takes an optional `today` to sharpen the YTD label / `stepDay` / `yearsWithData`) + the truncating widget formatters (`truncDp` / `fmtDec2` / `fmtPct2` / `fmtRM2` / `fmtHM`) and `widgetPeriod` / `widgetPeriodLabel`. Split from the `.tsx` so Fast Refresh keeps working | | | |
