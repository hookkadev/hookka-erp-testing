...
> `rack-scan` / `public-rack-qr.ts` row (DEV-09 movement name / label / TRANSFER, `/m` Scan Rack + Find) re-derived.
...
> **Restamped 2026-09-22 on branch `chore/sync-staging-from-main` (PR #463, staging ← main):** re-checked
> mechanically by `node scripts/check-codebase-map.mjs` after the merge — 4 drifted refs rewritten
> (`worker.ts` middleware lines moved +12 with staging's KV-prefix middleware; `checkPoRemaining`
> `purchase-invoices.ts:966`→`:987`); the auto-fix's `:29` (the import) re-pointed to the `app.use` at `:939`.
> `docs/API.md` regenerated (141 mounts, 971 handlers). Staging's `src/pages/test/` dashboard is gone —
> every dashboard row below describes `src/pages/dashboards/` + `/dashboard-experimental`, which is what
> the synced branch runs.
...
> Discount form also gained a date field (POST `date`, `<input type="date">`).
...
> **Last verified: 2026-09-24 on branch `fix/scan-code-family-match`** — the Procurement gotcha "Scan line → internal code resolves as" only (code-family step, BUG-2026-09-24-189).
>
> **Last verified: 2026-09-24 on branch `fix/pi-document-discount`** — the Procurement gotcha "PI discount" only (reshaped to one invoice-level discount, BUG-2026-09-24-188). Earlier the same day on `feat/pi-line-discount-main`: that gotcha (new, DEV-14) and the `checkPoRemaining` anchor.
>
...
>
> **Last verified: 2026-09-25 on branch `feat/dashboard-efficiency-employee-drill`**: EmployeesView.tsx row (Efficiency sub-tab now opens with `DailyEfficiencyCard`, which drills into the `emp`-selected worker) and dashboard-shared-lib.ts row (`workerDays`) checked against the code. Nothing else re-checked.
>
> **Last verified: 2026-09-25 on branch `feat/employees-kpi-layout`, PR #530**: dashboard-shared-lib.ts row (`overallEfficiencyPct` skips unclocked days, now also used by /m Operations) checked against the code.
>
> **Last verified: 2026-09-25 on branch `feat/employees-kpi-layout`**: EmployeesView.tsx row (Overview down to two tiles, target folded into the efficiency tile) and dashboard-shared.tsx row (`Kpi` `hint` / `children`) re-checked against the code. Nothing else re-checked.
>
> **Last verified: 2026-09-25 on branch `feat/dashboard-kpi-no-icons`** (PR #524): Experimental dashboard "Access" paragraph rewritten for the per-role tab map (PRODUCTION); rows for dashboard-prototype.tsx (card look, period labels), SalesOrdersView.tsx, OperationsView.tsx, EmployeesView.tsx, dashboard-shared.tsx (`Kpi` without icon), dashboard-shared-lib.ts (`overallEfficiencyPct`), dashboard-b/index.tsx and working-hour-entries.ts (summary carries names) corrected against the code; line counts re-measured with `wc -l`.
...
| **Make-to-stock (DEV-05, 2026-09-17)** — `StockAllocationCard` in `sales/detail.tsx` (allocate / release from stock); per-line "N available from stock" on `sales/create.tsx` | `src/api/routes/stock-allocations.ts` — `GET /availability`, `GET /?salesOrderId`, `POST /` (allocate), `POST /release`. Logic in `src/api/lib/stock-allocations.ts` (ledger, availability, whole-order ownership transfer) + `src/api/lib/stock-orders.ts` (`is_stock` / `stock_origin_so_id` self-apply, `cust-factory-stock`). Auto-allocation rides SO confirm (`sales-orders.ts` `POST /:id/confirm`) in the same batch | `stock_allocations` (append-only; migration 0237) / `sales_orders.is_stock` / `production_orders.{is_stock,stock_origin_so_id}` (0236) / `customers` row `cust-factory-stock` | `tests/stock-allocations.test.mjs` (17), `tests/delivery-pipeline.test.mjs` (R2/R12 gate) |
...
- **Make-to-stock allocation is WHOLE PRODUCTION ORDERS changing hands, never a quantity split off one** (DEV-05). `POST /production-orders/stock` builds one order per piece (SOFA: one per set), exactly as `production-builder.ts:518-520` builds a customer order — it used to be the only place creating a single order carrying the whole quantity. "Still unallocated" is `production_orders.sales_order_id = stock_origin_so_id`; allocation moves `sales_order_id` to the customer (+ a `stock_allocations` row in the SAME batch — never bare), and from then on the piece flows through delivery / the DO one-customer check / invoice-so-item-link with NO special case. `is_stock` is never cleared: it records birth, not ownership. Availability is RECOMPUTED from production_orders, never summed from the ledger (test-pinned). A stock SOFA set goes out whole (owner 2026-09-17). Release is refused once the piece is on a live delivery order (Q5). The SO list and `/stats` exclude `is_stock` rows by default (`?isStock=all`).
...
| `src/pages/procurement/detail.tsx` — PO detail + ThreeWayMatchPanel (1497). Line Price (RM) is `MoneyInput` since 2026-09-22 (BUG-2026-09-22-179: the raw input reformatted on every keystroke) | `src/api/routes/grn.ts` — GRN CRUD + arrival + Post-to-Stock cascade | `grns` / `grn_items` | `tests/ocr-distill-supplier.test.mjs` |
| `src/pages/procurement/create.tsx` — full-page PO create; the Supplier header field is a `SearchableSelect` (type code or name; clear entry resets purchase company to HOOKKA), no longer a native `<select>`. Line Price (RM) is `MoneyInput` since 2026-09-22 (BUG-2026-09-22-179) | `src/api/routes/goods-in-transit.ts` — GIT CRUD | `goods_in_transit` | `tests/supplier-payment-alloc.test.mjs` |
...
- **PI discount (DEV-14, 2026-09-24; reshaped by BUG-2026-09-24-188)**: the operator enters ONE invoice-level "Less: Discount" (create footer + summary, detail edit footer, scan card — pre-filled from the OCR footer `discount`); `allocateDiscountSen` (`src/lib/unit-price.ts`, largest-remainder, sums to the sen, TAX lines excluded) spreads it pro-rata into `purchase_invoice_items.discount_sen` (self-applied in `ensurePiMigrations`). There is NO per-line Discount column on any surface — suppliers print the discount once, at the bottom. The API stores `line_total_sen` NET of each share via `discountedLineSen` (clamped to [0, gross]), so every `line_total_sen` reader (GL legs, costing, 3-way match, AP) sees the discounted amount unchanged; view + PDF show line amounts at gross and the invoice discount = Σ `discount_sen` in the totals. Distinct from the `DISCOUNT` line TYPE, which is a separate line and (pre-existing) ADDS to the total. Scan: `sanitizeSupplierDoc` adds the discount back when it backs a unit price out of a net amount. Tests: `tests/pi-line-discount.test.mjs`.
...
**Big-file section index**
- `src/pages/delivery/index.tsx` (anchors re-derived 2026-09-22)
...
- **DO list is paginated PER STAGE TAB (2026-09-22, BUG-2026-09-22-003).** `doBrowseUrl` sends `GET /api/delivery-orders?status=<that tab's statuses>&page&limit=50`; the route's paginated branch binds them as `AND status IN (...)`. Before, every tab paged the newest 200 DOs of EVERY status and filtered client-side — "Delivered" showed only the delivered rows inside that window, "Page 1 / 3" counted all statuses, and PO tabs fetched 200 DO rows they never showed. Whole-table figures (tab counts, tab money, **Delivered (MTD) = `/stats.deliveredMtd`**, counted server-side in Malaysian time via `startOfMonthMYT`) never come from the browse page. Tab changes go through ONE `useUrlBatch` write (`goTab`: tab + page reset) — the old `setPage(1)` effect fired a second navigation per click — and scroll restore writes sessionStorage directly instead of re-rendering the page on every scroll event. Helpers: `src/lib/delivery-list-filters.ts`. Tests: `tests/delivery-list-filters.test.mjs`. **The Pending Delivery FREEZE (BUG-2026-09-22-005, C24)** was a render loop between `DataGrid.onSelectionChange` and `pendingDeliveryColumns`' deps — fixed on both sides, see the data-grid bullet. **The CN mirror (`consignment/note.tsx`) still pages globally** — its API takes a single `?status=`; parity is a follow-up.
...
- `src/pages/accounting/index.tsx`
  ...
  - Payment Vouchers tab (`PaymentsTab`, anchor by name — ~L8536 as of 2026-09-22; `ReceiptsTab` follows at ~L9536) — ONE door for money out (Houzs adoption 2026-09-22): **New AP Payment** (`pvKind AP` — pays a supplier's purchase invoices + optional advance, or an other creditor's bills; `GET /payment-vouchers/open-bills` lists the creditor's payable docs minus amounts already ticked on other unposted vouchers; ticks live in `payment_voucher_allocs`; at APPROVE / Post now the settlement is written by `buildSupplierPaymentCreate` (supplier-payments.ts) / `buildOtherPartyPaymentCreate` under `paymentNo = pvNo`, so aging/reco/GL see an ordinary payment; cancel delegates to `buildSupplierPaymentLifecycle` / `buildOtherPartyPaymentLifecycle`; posted AP voucher has no in-place edit) and **New Payment Voucher** (expense lines, classic). Four-tier ladder Draft→Prepared→Checked→Approved on both; Houzs chips All/Draft/Prepared/Checked/Approved/Advance open/Cancelled; `?pay=<PI|AP>:<docId>:<partyId>` deep-link from AP Invoices ticks that bill. Backend: `ensurePvApColumns` (~accounting.ts:9378, records 0235/0216), `validatePvAp` (~:9537), `pvApSettlementStatements` (~:9588). Guard `tests/pv-ap-payment.test.mjs`. **Double-click detail (2026-09-22, every document list)**: `DocDetailModal` / `DetailField` shell — Payment Vouchers (ladder trail + lines/bills + attachments + the row's actions), Receipts hub (shared `detailTable` for inline expand + popup), Fund Transfer, Other Party Bills (toggles the full card), Other Party Payments, AP Invoices (PI → Procurement page, AP bill → editor); single click keeps select / expand; guard `tests/doc-detail-dblclick.test.mjs`. **Attachments + print bundle (2026-09-22)**: files ride the generic store (`file_assets`, resourceType `payment_voucher`) through files.ts's one upload/delete path (`storeUploadedFile` / `removeStoredFile`); `GET/POST /payment-vouchers/:id/attachments`, `DELETE …/:fileId` (cancelled = no new files; delete only Draft/Prepared — evidence locked from Check); Scan Bills / Scan Receipt attach the scanned file; row `print + files` = `printPvBundle` (images as-is, PDFs via pdfjs → `VoucherSpec.appendix` pages; any failure refuses the print). Guard `tests/pv-attachments.test.mjs`.
  - Receipts hub (`ReceiptsHubTab`, anchor by name; replaced the single-door `ReceiptsTab` 2026-09-22 「receipts 三门合一」) — ONE door for money in: **New Customer Receipt** (`CustomerReceiptForm`, exported from `src/pages/invoices/payments.tsx` and rendered by that page too — settles sales invoices via `/api/payments`), **New Other Debtor Receipt** (`OtherPartyPaymentForm`, the settle form extracted from `OtherPartyPaymentsManager`, side DEBTOR → `/other-party-payments`), **New Official Receipt** (`OfficialReceiptForm` → `/official-receipts`); one merged list (CUST / OD / OR badges, chips All/Customer/Other debtor/Official/Cancelled, print / edit / void routed per document). Customer voucher + advance helpers: `src/lib/customer-receipt.ts`. Guard `tests/receipts-hub.test.mjs`.
  ...
  - Daily Cash Position tab (`DailyCashTab`, anchor by name; `GET /cash-position`, `POST /cash-position/tick`) — per bank/cash account: Bank balance (est.) = book ± pending; each pending leg has a ✓ (kv `bank_board_cleared`, real statement overrides the tick). **2026-09-22**: ticked rows leave the pending list and fold under a per-account line 「✓ Ticked as gone through: N · out / in」(open to review or untick); the figures never read the DOM. TO REPAY / TO RECEIVE panels, planned payments (default off), AWAITING APPROVAL card (checked-not-approved vouchers), Get image modal + Print. Guard `tests/cashpos-ticked-fold.test.mjs`.
...
- **Special-order surcharge = ONE function** (BUG-2026-09-23-183, 2026-09-23). All four order forms (`sales/{create,edit}.tsx`, `consignment/{create,edit}.tsx`) price ticked specials via `calcSpecialsSurchargeSen(codes, maintenanceConfig.specials|sofaSpecials)` and label them via `specialNameForCode` in `src/lib/special-order-surcharge.ts`. Options added in Settings (config-only, slugged codes) are priced from the config — they used to add RM 0. Server: sofa lines price from `loadSpecialsConfig(db, "sofaSpecials")`. Existing rows: `GET/POST /api/admin/backfill-config-only-specials` (`repriceSavedSpecialsLine`). Tests: `tests/special-order-surcharge.test.mjs`, `tests/special-order-combo-discount.test.mjs`.
...
- Supplier Discount (purchase CN) = `accounting.ts` `/purchase-credit-notes` POST(DRAFT)+PUT(POSTED, optional `allocations[]`)+`/:id/void`+`/:id/restate`; UI `SupplierDiscountTab` (tab `supplier-discount`). The CN's GL is DR400/CR-purchase (the ONLY GL move). Knocking it off a PI does NOT add a GL leg — it bumps the PI's `paid_amount_sen` + writes a `supplier_payments` `method='CREDIT_NOTE'` marker (`amountSen=0`, `bookedSen=applied`, `paymentNo=<CN no>`). Markers are EXCLUDED from the supplier-payment history list; `/ap-control` nets only the UNALLOCATED CN remainder (`Σ posted CN − Σ marker bookedSen`) over net PI outstanding so drift stays 0. Void deletes markers + restores paid_amount. Allocation math: pure `src/lib/discount-alloc.ts` (#6). POST accepts an optional `date` (`YYYY-MM-DD`, falls back to today) — the entry form has a native `<input type="date">`.
- **Void 500'd on prod (2026-09-18, BUG-2026-09-18-001, same class as BUG-2026-06-25-001).** `purchase_credit_notes.status` was created `CHECK (status IN ('DRAFT','POSTED'))` (migrations/0088, migrations-postgres/0156) — no `CANCELLED`, yet the void handler has always written `status='CANCELLED'`, so every void hit the constraint and 500'd. Fixed by `ensurePcnCancellable` (`accounting.ts`, right above the void route) — a runtime self-apply that drops the old constraint and re-adds it with `CANCELLED`, run at the top of the void handler. Test: `tests/pcn-void-status-check.test.mjs`.
- **History row actions are Edit + Cancel (2026-09-21, owner UX ask + naming rule "every button reads Cancel"), not a raw Void button.** `POST /purchase-credit-notes/:id/restate` edits a POSTED note IN PLACE — same shape as the other-party-bill / payment-voucher restate: reverses whichever GL legs are currently `hidden = 0` (the original post, or a prior restate — never more than one live set) under `purchase_credit_note_restate_rev:<stamp>`, posts the corrected legs under `purchase_credit_note_restate_post:<stamp>`, hides everything else. **Allocations are frozen across an edit** — the edited gross is refused if it would drop below `Σ booked_sen` already knocked off PIs; reallocating means Cancel + re-create, not Edit. The void handler (kept as `/void` server-side; the button just says "Cancel") is now restate-aware the same way — it reverses whatever is currently visible, then hides the lot, so cancelling an edited note doesn't leave a stale pre-edit leg behind. `ledger_journal_entries.hidden` (mig 0177) is a defensive self-apply inside `ensurePcnCancellable` now, not assumed present. Edit is INLINE in the history row (date / net / SST / reason inputs + Save / Discard in that row — `saveEdit`), not the top entry form. A cancelled note stays in the history list showing its `CANCELLED` badge (only `DRAFT` orphans are hidden); its Edit/Cancel buttons are gone.
...
- **Overview matrix tablet rule (2026-09-22):** below 1280px (`isTablet`, `src/pages/production/index.tsx` ~L559) the Overview drops Customer PO / Special Order / Our Expected DD (`OVERVIEW_TABLET_HIDDEN`) and uses `OVERVIEW_TABLET_WIDTHS` (dept cols 72px) — same rule the dept DataGrids already apply via `defaultHidden: isTablet`. Wide: 18 tracks / 1,980px min; tablet: 15 tracks / 1,220px min. Header + body cells are hand-written per column, so a hidden column must be gated with `!ovHidden` in BOTH places or the tracks misalign. Phones are NOT this page's job — `/m/production` (`src/pages/m/screens/ProductionScreen.tsx`) is the only small-screen surface; don't add a third layout.
...
| | `src/api/routes/warehouse.ts` — racks + movements (864); `GET /locate?q=` = DEV-09 per-piece "where is it" search, `warehouse:read` gated (`tests/warehouse-scan-history.test.mjs`) | `fg_stock_events` (append-only FG ledger, mig 0221) | `tests/fg-stock-events.test.mjs` |
...
| Frontend page | API route | Primary tables | Tests |
|---|---|---|---|
| `src/pages/dashboard-b/index.tsx` — the entire Command Center (2667); Worker Efficiency card reads names and departments from `/api/working-hour-entries/summary`, not `/api/workers` (BUG-2026-09-25-194); KPI rail + month switcher + all widgets | `src/api/routes/dashboard-overview.ts` — single GET / (2249), 60s KV-cached, owns ALL dashboard data | `sales_orders` / `sales_order_items` | `tests/snapshot-freshness.test.mjs` |
...
as `/dashboard` (`/api/dashboard/overview?period=`, `/api/sales-orders/stats`,
`/api/delivery-orders/pending-value`, `/api/delivery-orders/stats`) through `useCachedJson` and
derives its figures in `src/pages/dashboards/dashboard-widgets-lib.ts` — pure functions that MIRROR
the inline formulas in `dashboard-b/index.tsx` (that file is frozen by the owner and does NOT
import the lib; change a formula there → change it in the lib). Period: `widgetPeriod` maps YTD →
`all`, anything else → its month; every card title carries `widgetPeriodLabel`. **No rounding:**
money `fmtRM2`, % `fmtPct2`, decimals `fmtDec2`, minutes `fmtHM` — all TRUNCATE to 2dp
(`truncDp`, `dashboard-shared-lib.ts`); `deptBacklogRows`/`plantLoad` take an `unrounded` flag
that recomputes days from minutes instead of the server's 1dp figure. A failed load prints
"Couldn't load … — not shown as zero". Placement: Overview = Invoices (5th hero), Daily Report
(5th "Needs action" tile, `complianceSummary` + `isUnknownOutcome`), `OcrAccuracyCard` at the
bottom; Sales = Order Pipeline, Revenue trend, Sales by Customer, Top Sellers (before
"Fulfillment pipeline"); Operations > Overview = Plant Load (dial) + Department Backlog (per-dept status board, 2026-09-24 —
it replaced the Overdue-by-dept chart), > Output = Completed, > Materials = Purchasing + Fabric Usage; Finance >
Returns = `AgingRatiosCard` (plain `fetch`, never `useCachedJson`: gross margin / current /
quick ratio from `/api/accounting/dashboard` rows via `financeRatios`, unrounded — the server's
`ratios` are rounded; AR/AP 5-bucket aging from `/api/accounting/aging` via
`agingBucketTotals` in `src/lib/aging-export.ts`, labelled live as of today; 401/403 → "Needs
...
| Frontend page | API route | Primary tables | Tests |
|---|---|---|---|
| `src/pages/dashboards/dashboard-prototype.tsx` — page shell: tab strip + the GLOBAL period control (Day / Month / YTD, redesigned 2026-09-22) that every dated panel reads. **Look (2026-09-25):** cards on this page are flat with a smaller radius and only get a shadow on hover. The root div restyles `[data-slot=card]` (the attribute `Card` in `src/components/ui/card.tsx` now carries), so other pages keep the default Card. Every tab shows the selected period next to its heading. Tabs are named by FUNCTION, never by staff member (2026-09-21): Overview · Sales · Operations (`OperationsView.tsx`) · Employees (key `people`, relabelled 2026-09-22; `EmployeesView.tsx`, sub-tabs Overview / Time & attendance / Efficiency — the Departments sub-tab was folded into Efficiency 2026-09-24: `DepartmentsView.tsx` is now just the department-ledger card, fed the filtered slice, between `DeptEfficiencyCard` and `EfficiencyPanels`; `?tab=people&sub=departments` and `?tab=department` resolve to people/efficiency via `LEGACY`, which `/m`'s `useDashboardSub` also reads) · Service · Finance — the same keys as the `/m` dashboard. No reviewer tab: each chart has one home, approvals live once in Service > Approvals, and Overview's "Needs action" strip deep-links there (`onOpenTab(tab, sub)`). Old `?tab=siti|lim|employee|department` links resolve through `LEGACY` in `dashboard-url-state-lib.ts`. **Phone width (<768px):** the tab strip is a native `<select>` (here and on `/m/dashboard`), and `PeriodPicker` (`dashboard-shared.tsx`) collapses to ONE button that opens a bottom sheet (Day/Month/YTD, a stepper that moves by whatever view is active, presets in Day view, a 12-month grid in Month view) portalled to `<body>` — the sticky header's `backdrop-blur` would otherwise become the containing block of the fixed sheet. A real phone never sees that picker (`DashboardLayout` redirects mobile devices to `/m`): its period control is `src/pages/m/screens/dashboard/PeriodChip.tsx`, same features, `/m` theme. Both run on the pure `stepPeriod` / `stepDay` / `yearsWithData` / `periodPresets` / `presetActive` / `calendarCells` / `shiftMonth` in `dashboard-shared-lib.ts` (test `tests/dashboard-period.test.mjs`). **Day/Month/YTD nav (2026-09-22):** which of the three views a period reads as is DERIVED from the existing `Period` shape (no new mode) - `mode==="ytd"` -> YTD, a range preset or a set `day` -> Day, else -> Month; Day's arrows step by calendar day (capped at today), Month's by the neighbouring month WITH DATA (unchanged `stepPeriod`), YTD's by year (unchanged, already disabled with one year). Month's popover is a new `MonthGrid` (year-stepped 12-month grid, `dashboard-shared.tsx`); YTD has no popover, its arrows cover everything. **Opens on TODAY (2026-09-22):** `resolvePeriod(period, months, today)` in `dashboard-shared-lib.ts` (desktop shell + `/m` `hooks.ts`) turns a BARE URL into this month with `day` = today; every dated figure narrows to `period.day` through `inFocus` (Overview totals compare the day with the day before, `dashboard-sales-lib.ts`), trend charts keep the whole month, and clearing the highlight writes `month=` so it stays cleared. Finance is a monthly ledger and does not follow the day. **Overview and Sales open on the MONTH, not today (owner 2026-09-21):** `opensOnToday(tab)` in `dashboard-shared-lib.ts` feeds `resolvePeriod`'s 4th arg `openOnToday` (desktop shell + `/m` `useDashboardPeriod(months, tab)`); a day the user picks there is still honoured (test in `tests/dashboard-m-lib.test.mjs`). **Navigation state lives in the URL (2026-09-21):** tab/sub-tab/period are query params (`?tab=operations&sub=plan&mode=range&month=&from=&to=&lbl=&day=`, defaults omitted) via `use-dashboard-url-state.ts` + pure `dashboard-url-state-lib.ts` (`TAB_SUBS` registry: new tab with sub-tabs = one line; test `tests/dashboard-url-state.test.mjs`); tab/sub push history, period/day replace | `src/api/routes/dashboard-prototype.ts` — single `GET /` (line 293, file 1975 lines), mounted at `/api/dashboard/prototype` (`src/api/worker.ts:1314`), auth-gated, 60s KV-cached (2026-09-17, same `cached()` helper as dashboard-overview.ts — the route was ~15 sequential queries with no caching, ~1.5-3s cold). The per-request permission redaction (delivery/purchase/inventory/employee/production sections dropped per caller) runs OUTSIDE the cache, on every request, against the cached raw payload — never cache the redacted response itself. `production`/`inventory` gained the Siti-list fields (2026-09-17): `overdueByDept`, `dueSoon3Days`, `dailyOutput`, `planVsActual` derive from data the route already loads (plus one extra SELECTed column, `completed_date`, on the existing production_orders query); `productionCost` is the one genuinely new query — reads `fg_batches` (materialCostSen/laborCostSen/overheadCostSen, no org_id filter — that table carries none anywhere in this codebase), and most batches read RM 0.00 because fg-completion.ts creates them at zero and only po-cost-cascade.ts fills in a real figure later — `batchesWithCost`/`totalBatches` say how many are real; `materialShortage` (proxy: active RM at zero/negative balance, since min_stock is unset on every row). Returns all six sections in one payload | `sales_orders` / `sales_order_items` | none yet — see gap below |
| `src/pages/m/screens/dashboard/` — the PHONE dashboard (`/m/dashboard/:tab`, More menu + left rail entry "Dashboard Prototype", gated by the desktop `/dashboard-experimental` nav entry). `DashboardScreen.tsx` shell (tab `<select>` + `PeriodChip.tsx` sheet), `registry.tsx` maps each tab key to `tabs/OverviewTab.tsx` · `SalesTab.tsx` · `OperationsTab.tsx` · `PeopleTab.tsx` (label "Employees") · `ServiceTab.tsx` (approvals = the desktop panel's endpoints, results inline: `/m` has no ToastProvider) · `FinanceTab.tsx` (plain `fetch`, never the localStorage cache). Sub-tabs: `useDashboardSub` (`hooks.ts`) + `MSubPills` (`primitives.tsx`), same `?sub=` keys as desktop | ↳ same feed as the desktop tabs; Finance reads `/api/dashboard/finance`; approvals read/write `/api/service-cases/approvals` | same as the desktop views | `tests/dashboard-m-lib.test.mjs`, `tests/dashboard-period.test.mjs` |
| `src/pages/dashboards/ServiceCaseLink.tsx` + `use-service-case-links.ts` + pure `service-case-link-lib.ts` — **Service tab → Service Cases deep links (2026-09-21):** every per-case row (Report list, Overdue, Performance > prevention-not-done, Approvals) opens the real module — desktop `/service-cases/:id` via `<ServiceCaseNo>` (a `<Link>`; table rows also navigate on click via `rowProps`, which ignores clicks on links/buttons/inputs), `/m` `/m/servicecases/:id` via `ListRow onClick` + `navigate` in `tabs/ServiceTab.tsx`; both headers carry an "Open Service Cases" link to the list page. Gated by `isNavAllowed("/service-cases")` (the sidebar entry) — not allowed = plain text. In Approvals only the case NUMBER links, never the row (it holds Approve/Reject). Issues panels are aggregates, nothing to link | ↳ same feed — `service.cases[].id` was already there; no API change | `service_cases` | `tests/dashboard-service-case-link.test.mjs` (also pins the hrefs to the router sources) |
| `src/pages/dashboards/DashboardWidgets.tsx` — the widgets ported from `/dashboard` 2026-09-23 (see "Widgets from `/dashboard`" above): `OrderPipelineCard`, `RevenueTrendCard`, `CustomerRevenueCard`, `TopSellersCard`, `PlantLoadCard`, `DeptBacklogCard`, `CompletedCard`, `PurchasingCard`, `FabricUsageCard`, `AgingRatiosCard`. Formulas in `src/pages/dashboards/dashboard-widgets-lib.ts` (mirror of `dashboard-b/index.tsx`) | `/api/dashboard/overview`, `/api/sales-orders/stats`, `/api/delivery-orders/pending-value`, `/api/delivery-orders/stats`, `/api/accounting/dashboard`, `/api/accounting/aging` | same as `/dashboard` | `tests/dashboard-widgets-lib.test.mjs` |
| `src/pages/dashboards/AllOverviewView.tsx` — landing tab: 5 hero KPIs (5th = Invoices, as on `/dashboard`) + "Needs action" (5th = Daily Report tile, links `/daily-report`) + 5 domain summary cards + `OcrAccuracyCard` | ↳ same feed, `sales` / `production` / `delivery` / `inventory` / `employee` slices | `production_orders` / `job_cards` / `delivery_orders` | |
| `src/pages/dashboards/SalesOrdersView.tsx` (1221) — KPIs w/ deltas (Outstanding, Pending Delivery and Completed also show their RM value: `outstandingSen` / `pendingDeliverySen` / `completedSen` in `computeSalesKpis`, `dashboard-sales-lib.ts`), revenue trend (bars = revenue, line = orders, Both / Revenue / Orders switch top right, click-to-drill), attribution by customer, 3-month rolling forecast, `/dashboard` widgets (Order Pipeline / Revenue trend / Sales by Customer / Top Sellers, from `DashboardWidgets.tsx`), state/category donut, Top SKUs | ↳ same feed, `sales` slice only | `raw_materials` / `rm_batches` / `purchase_orders` | |
| `src/pages/dashboards/OperationsView.tsx` — Operations tab (sub-tabs Overview / Output / Plan vs Actual / Revenue & Cost / Materials; Plan vs Actual and production revenue are mounted from `ProductionDailyPanels.tsx`, which also exports `DeptEfficiencyCard` for Employees > Efficiency), Overview (re-laid-out 2026-09-24): the 7 KPIs in three labelled groups — Orders (Overdue / Due-3-Days / Plan-vs-Actual stage-completion % with progress bar) on top, Materials & cost (Shortage / Production Cost) and People (Attendance / Efficiency with 7-day sparkline; the figure comes from the same `overallEfficiencyPct` as Employees > Overview so the two agree) side by side below at xl; then `PlantLoadCard` as a half-ring dial (0–14d, notches at plantLoad()'s 7d/12d cut-offs, per-day capacity table folded under a line chart) beside `DeptBacklogCard`, now a per-department status board joining backlog days (by name, `/api/dashboard/overview`) with LIVE overdue + due-in-3-days counts (by job-card code, prototype feed) through `deptStatusRows` in `ops-floor-lib.ts` (code→name table mirrors `DEPARTMENTS` in `dashboard-overview.ts`; test `tests/ops-floor-lib.test.mjs`); then `DueSoonWorklist` (`OverdueCards.tsx`) as urgency lanes (Today / Tomorrow / 2 / 3 days, click-to-filter, dept pills + search kept); then `AttendanceLogCard` (header and "Listed rows" totals row are sticky on their CELLS, not the `<tr>`, BUG-2026-09-25-193; every dashboard table header follows the same pattern), which has its own Today / Yesterday chips (card-local override of the page period, reset when the page period changes; the feed is not date-windowed so yesterday is always present). `OverdueByDeptCard` was removed. `/dashboard` widgets (2026-09-23, `DashboardWidgets.tsx`): Overview = Plant Load + Department Backlog, Output = Completed, Materials = Purchasing + Fabric Usage | ↳ same feed, `production` (`orders` stagesDone/stagesTotal, `overdueByDept`/`dueSoon3Days`/`dailyOutput`/`planVsActual`/`productionCost`) + `inventory.materialShortage` + `employee` (attendance/performance, redaction-gated) | `production_orders` / `fg_batches` / `raw_materials` / `attendance_records` / `working_hour_entries` | none yet |
| `src/pages/dashboards/EmployeesView.tsx` — headcount + "Overall Efficiency" (total production minutes ÷ total working minutes via `overallEfficiencyPct` in `dashboard-shared-lib.ts`, a weighted total, NOT an average of people and NOT `attendance_records.efficiency_pct`; see note below) KPIs. Overview has TWO tiles since 2026-09-25: the efficiency tile carries `meta.config.efficiencyTargetPct` as a "Target: N%" line plus a bar against it, and `workingHoursPerDay` is a "Standard working day" line under the grid (the old Target / Working Hours tiles are gone); scope notes are `Kpi` `hint` tooltips. Then the working-vs-production-hours chart (scoped to the global period picker), worker table. **Efficiency sub-tab (2026-09-25):** sticky filter bar, then `DailyEfficiencyCard` (`EmployeesInsights.tsx`, shared with Time & attendance): pool efficiency per day, or with the `emp` filter set, that worker for every day of the period (working vs production hour bars + their efficiency line, via `workerDays`); "← All employees", Reset and clearing the Employee select all go back. Top/Bottom 5 and warning-audit rows call `pickEmployee` (sets `emp`, scrolls the chart into view; keyboard operable). Then `DeptEfficiencyCard`, `DepartmentsView`, `EfficiencyPanels`. Denied (no `workers:read`) renders via `availability.employee` same as a query failure | ↳ same feed, `employee` / `availability.employee` slice only | `workers` / `working_hour_entries` / `job_cards` | none yet |
| `src/pages/dashboards/dashboard-shared.tsx` — `PeriodPicker` (Day/Month/YTD toggle + arrows + its trigger popover: `CalendarGrid` for Day, `MonthGrid` for Month, none for YTD), `Kpi` (no icon since 2026-09-25: label, value, then sub; a `+x% / -x%` sub renders as a green/red pill, any other sub as plain wrapping text; optional `hint` = info-icon hover tooltip after the label, optional `children` rendered under the sub, e.g. a baseline bar), `LiveBadge`, `MissingNote`, `SnapshotNote` | | `attendance_records` / `working_hour_entries` / `workers` | |
| `src/pages/dashboards/dashboard-shared-lib.ts` — chart theme tokens + the `Period` model (`inPeriod` / `previousPeriod` / `periodLabel` - `periodLabel` takes an optional `today` to sharpen the YTD label / `stepDay` / `yearsWithData`) + `workerDays` (one worker's hours for every calendar day of a monthly/range period, days worked only for YTD; the Efficiency drill-down; test `tests/dashboard-period.test.mjs`) + `overallEfficiencyPct` (Employees + Operations efficiency on desktop and /m; skips days with no clocked production time so today's earned minutes do not inflate the month, BUG-2026-09-25-195; test `tests/dashboard-period.test.mjs`) + `dayLabel` / `dayList` and `warnDays` (the dates behind a time-audit warning, used by `EmployeesInsights.tsx` and `/m` `PeopleTab.tsx`) + the truncating widget formatters (`truncDp` / `fmtDec2` / `fmtPct2` / `fmtRM2` / `fmtHM`) and `widgetPeriod` / `widgetPeriodLabel`. Split from the `.tsx` so Fast Refresh keeps working | | | |
| `src/pages/dashboards/FinanceView.tsx` — **Finance tab** (sub-tabs Per head / Returns & balance sheet / Outlook & P/E; avg employee cost + revenue, ROA/ROE/ROI, liabilities vs assets, YoY, 12-month forecast, P/E with owner-entered valuation; Returns also mounts `AgingRatiosCard` — margins / current & quick ratio / AR-AP aging, 2026-09-23). Own plain `fetch` (no localStorage cache — figures are sensitive), NOT the shared feed. Last verified 2026-09-23 | `src/api/routes/dashboard-finance.ts` — `GET /api/dashboard/finance?mode&month` (accounting `read`, `no-store`) + `PUT /valuation` (accounting `update`); pure maths in `src/api/lib/dashboard-finance.ts`; ledger rows from `loadFinanceSeries` in `accounting.ts` (per month: P&L via `computePnlWindow`/`pnl_historical`, BS via the `/pl` balance-sheet legs + `bsSectionFor`); forecast maths shared with the Sales tab in `src/lib/revenue-forecast.ts`; headcount rule in `src/api/lib/headcount-rule.ts` | `ledger_journal_entries` / `chart_of_accounts` / `workers` / `kv_config` key `company_valuation_sen` (integer sen) | `tests/dashboard-finance.test.mjs` |
...
| `src/pages/rack-scan.tsx` — rack QR stock-in; carries pieceNo/totalPieces per line (1249) | `src/api/routes/public-rack-qr.ts` — PUBLIC no-login rack stock-in + /p/ piece-sticker rack-write (auth-bypassed, idempotent); /item + stock-in are PER-PIECE (pieceNo+totalPieces → distinct rack_items row; multi-piece stamps piece_pics.racking_number not card-level); stock-in movement = session user name (`scannerName`) else "Public scan", rack LABEL, cross-rack move = `TRANSFER` "Moved from <rack>" (DEV-09). `/m/warehouse` (`src/pages/m/screens/WarehouseScreen.tsx`) Scan Rack → navigates to `/r/<id>`, Find tab → `GET /api/warehouse/locate` | `kv_config` / `hookka_erp_metrics` / `piece_pics` | `tests/rack-qr-per-piece.test.mjs` / `tests/scan-per-piece.test.mjs` |
...
- **Idempotency is the opposite of CSRF: OPT-IN, per-call.** `withIdempotency` (`src/api/lib/idempotency.ts`) is a NO-OP when the request carries no `Idempotency-Key` header, so wrapping a route protects nothing by itself — T-006 R10 wrapped six create/convert routes and shipped with no screen sending a key, i.e. inert. The client half is `useIdempotencyKey` / `useIdempotencyKeys` (`src/lib/idempotency-key.ts`): it holds ONE key across an unanswered attempt (fetchJson reports a timeout/drop as status 0) and rotates it as soon as any response arrives — including 4xx, which the server caches deliberately, so a held key would replay an old rejection onto a corrected form forever. An inline `crypto.randomUUID()` at the fetch is NOT an idempotency key (fresh per call = protects nothing); a shared key across rows is worse (row 2 replays row 1's document — use the keyed variant with the row id). Double-clicks are stopped by the submit button's in-flight `disabled`, not by the key. Guarded by `tests/t006-r10-client-key.test.mjs`.
...
- **`DataGrid.onSelectionChange` emits ONLY when the selection changed (2026-09-22, BUG-2026-09-22-005, BUG-CLASSES C24).** `sortedData` is `[...filteredData]` — a fresh array on every recompute, and it recomputes when `columns` changes identity — so a parent that stores the emission as a fresh object AND memoises its `columns` on that state closes a render loop that freezes the page and starves every React Router 7 navigation transition. The grid now compares (same length, same row references, same order) before calling back; a data refresh still emits because the row objects are new. Parent rule: never list selection state in the deps of `columns` / `data` you hand back to the same grid. Test: `tests/datagrid-selection-emit.test.mjs`.
...
`src/api/lib/auth-middleware.ts`. `customerScopeMiddleware` (`src/api/worker.ts:939`),
`tenantMiddleware` (`src/api/worker.ts:946`) and `apiRateLimit` (`src/api/worker.ts:961`)
