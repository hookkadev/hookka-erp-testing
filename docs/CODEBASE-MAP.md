...
- **Special-order surcharge = ONE function** (BUG-2026-09-23-183, 2026-09-23). All four order forms (`sales/{create,edit}.tsx`, `consignment/{create,edit}.tsx`) price ticked specials via `calcSpecialsSurchargeSen(codes, maintenanceConfig.specials|sofaSpecials)` and label them via `specialNameForCode` in `src/lib/special-order-surcharge.ts`. Options added in Settings (config-only, slugged codes) are priced from the config — they used to add RM 0. Tests: `tests/special-order-surcharge.test.mjs`, `tests/special-order-combo-discount.test.mjs`.
