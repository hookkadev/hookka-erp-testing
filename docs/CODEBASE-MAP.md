...
> **Restamped 2026-09-25 on branch `feat/service-dashboard-rework`:** the dashboard Service row (`ServiceView.tsx`): Report list category column + category picker (desktop and `/m`); `causeLabel` now uses the case page's root-cause wording ("Transport / 3PL", "Customer (not our fault)", ...).
