...
> **Last verified: 2026-09-24** (branch `feat/t013-sequence-lock`, PRD T-013 R1–R15) against
...
> `src/api/routes/sheets-sync.ts:238`, `src/api/lib/rbac.ts:353` (`requireAdmin`), the four
...
> `loadFgStickers` / `packingStickerUrl` anchors, re-derived after the "Load all" gate was removed and the
> Overview virtualizer moved into `OverviewVirtualRows`.
>
...
| `requireAdmin` | `src/api/lib/rbac.ts:353` | SUPER_ADMIN / ADMIN only — the nine bulk status writers in `import-completion/*` |
