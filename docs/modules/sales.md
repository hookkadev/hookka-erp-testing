...
5. **Copy-from-source (draft picker)** — `CopyFromSourceModal` `create.tsx:2382` (2-step) + backend `app.post("/copy-for-service-order")` `sales-orders.ts:5321`.
...
- **Add a field to the SO** → column self-apply in `ensurePendingMigrations` (`sales-orders/_helpers.ts:1283`); persist in `app.post("/")` (`sales-orders.ts:1713`) and `app.put("/:id")` (`:3880`); surface in `rowToSO` (`_helpers.ts:243`) / `rowToSOList` (`:307`); render in `create.tsx:213` and `detail.tsx:338`. New column = snake_case (+ rename-map if camelCase).
