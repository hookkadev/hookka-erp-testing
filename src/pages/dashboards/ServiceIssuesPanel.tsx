...
//   - Cases by category  → a bar split closed | open, so "5 cases, all open" is visible
//   - Root cause         → the same split, one row per category + the detail recorded under it (byRootCause)
//   - Days to close      → one dot per closed case + the average, not one avg over 1-5 cases
//   - Category × day     → a heatmap on a fixed day grid; whole counts stay whole
...
function CloseDays({ cases }: { cases: IssueCase[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" role="img" aria-label="Days to close per closed case, by category" className="select-none">
      <title>Days to close per closed case, by category</title>
      {ticks.map((t) => (
        <g key={t}>
...
export function ServiceIssuesPanel({
  ...
  const causeCard = (
    <Card>
      <CardHeader className="pb-1">
        <CardTitle>Cases by category{causeOnly ? ` (${fmtN(cases.length)} cases)` : ""}</CardTitle>
        <Sub>
          The Category column of the Service Cases list, closed | open. A case with several categories counts once under each, so rows
          can add up to more than the total.{causeOnly ? "" : " Click a row to list those cases."}
        </Sub>
      </CardHeader>
      <CardContent><CauseSplit rows={causes} total={cases.length} onPick={causeOnly ? undefined : onPickCause} /></CardContent>
    </Card>
  );
  // One row per category + the detail recorded under it on the case's Root
  // Cause & Prevention panel (department, supplier, 3PL, salesperson or
  // sub-reason). Keys are "CATEGORY::detail", so no click-to-filter here — the
  // Report list filters by category only.
  const rootCauseCard = (
    <Card>
      <CardHeader className="pb-1">
        ...
        <Sub>
          ...
          3PL, salesperson or sub-reason), closed | open. A category with no detail recorded shows as one row.
        </Sub>
      </CardHeader>
      <CardContent><CauseSplit rows={rootCauses} total={cases.length} /></CardContent>
    </Card>
  );
  if (causeOnly) return <div className="space-y-5 max-md:space-y-4">{causeCard}{rootCauseCard}</div>;
  ...
  return (
    <div className="space-y-5 max-md:space-y-4">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5 max-md:gap-4">
        ...
        <Card>
          <CardHeader className="pb-1">
            ...
            <Sub>One dot per closed case (logged to closed, whole days); the tick is that category's average. Open cases are not drawn.</Sub>
          </CardHeader>
      ...
      <Card>
        <CardHeader className="pb-1">
          <CardTitle>Cases logged — category × {ytd ? "month" : "day"}</CardTitle>
