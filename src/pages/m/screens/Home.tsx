...
type WheSummaryResp = {
  data?: {
    ...
    // Resolved server-side (BUG-2026-09-25-193). null when the worker row is gone.
    workerName?: string | null;
    workerDepartmentCode?: string | null;
  }[];
};
...
export default function MobileHome() {
  ...
  // hours), merged like the desktop dashboard-b. Gated behind the same
  // `pdEnabled` idle flag so first paint stays the KPI rail. Window = current
  // month [1st..today].
  //
  // Unlike dashboard-b there is NO /api/workers fetch: the clocked-hours summary
  // carries each worker's name + department. /api/workers is gated on
  // workers:read (it returns payment columns), so a PRODUCTION account got a
  // 403 and the card showed raw ids — BUG-2026-09-25-193. ----
  ...
  const workerEff = useMemo(() => {
    ...
    for (const e of wheSumRaw?.data ?? []) {
      ...
      rows.push({
        name: e.workerName || e.workerId,
        dept: DEPT_LABEL[e.workerDepartmentCode || ""] ?? e.workerDepartmentCode ?? "",
